...
- **B站推荐回退**：仅当您主动开启功能时，ECHO 才会按 B站标签页在 `chrome.storage.session` 中保存最多 10 批结构化推荐，包括卡片链接、标题、封面、作者、时长、播放数和弹幕数。数据只存在于本地浏览器会话，按标签页隔离，不会上传或进入备份，并在标签页关闭时删除。
