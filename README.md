...
- **默认开启**，可与 Edge 自带拖拽同时使用、互不冲突，并额外支持拖拽链接打开新标签
- **拖拽文本**：自动使用 Bing 搜索
- **拖拽链接**：在新标签页打开
...
- **默认关闭**，由用户手动同步知乎官方黑名单
- 在知乎问答、评论与专栏中折叠黑名单用户本人发布的内容
- 提供临时查看入口；无法可靠识别作者时不隐藏
- 黑名单和账号快照只保存在浏览器本地，不进入备份或云同步

#### ↩️ B站推荐回退

- **默认开启**，在首页「换一换」旁提供上一批/下一批导航
- 按标签页会话保留最近 10 批推荐，刷新或同标签进入视频再返回后自动恢复，关闭标签页后清空
- 历史卡片使用结构化数据独立渲染，完整保存标题、封面、作者、时长、播放数和弹幕数，不保存 B站页面 HTML
